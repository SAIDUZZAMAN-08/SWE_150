...
bool initObstacles() {
    ...
    obstacles[0].rect = {655, 165, 60, 15};
    obstacles[1].rect = {SCREEN_WIDTH / 2 -300, SCREEN_HEIGHT / 2 - 120, 15, 350};
    obstacles[2].rect = {SCREEN_WIDTH / 2 + 300, SCREEN_HEIGHT / 2 - 120, 15, 350};
    obstacles[3].rect =  {100, 530, 60, 15};
...
void drawBonusFood(){
    if (isBonusFood) {
        ...
        imagesurface = IMG_Load("Star.png");
        imageTexture = SDL_CreateTextureFromSurface(gRenderer, imagesurface);
...
void drawScore() {
    ...
    SCREEN_WIDTH - highscoreTextSurface->w - 10, // Align to the right with 10px padding
    7,                                          // Vertical position near the top
    highscoreTextSurface->w,                    // Width of the surface
    highscoreTextSurface->h                     // Height of the surface
};
...
void renderStartScreen() {
    ...
    SDL_RenderClear(gRenderer);
     ...
     surface = SDL_LoadBMP("Cover_image.bmp");
	 Texture =SDL_CreateTextureFromSurface(gRenderer,surface);
	 SDL_FreeSurface(surface);
...
void handleBonusFoodTimer() {
    if (isBonusFood) {
        ...
        if (currentTime - bonusFoodStartTime >= 4000) { // Bonus food lasts for 4 seconds_ _ _Task_302
            isBonusFood = false;
        }
    }
}
...
void renderGameOverScreen() {
    ...
    surface = SDL_LoadBMP ("Over_image.bmp");
	 Texture =SDL_CreateTextureFromSurface(gRenderer,surface);
	 SDL_FreeSurface(surface);
	 SDL_RenderCopy(gRenderer,Texture,NULL,NULL);
    ...
    SDL_Rect gameOverRect = {SCREEN_WIDTH / 2 - gameOverSurface->w / 2-10, SCREEN_HEIGHT / 2 - 200, gameOverSurface->w+50, gameOverSurface->h+50};
    ...
    SDL_Rect scoreRect = {SCREEN_WIDTH / 2 - scoreSurface->w / 2, SCREEN_HEIGHT / 2 - 110, scoreSurface->w+20, scoreSurface->h+20};
    ...
    SDL_Rect OverRect2 = {SCREEN_WIDTH / 2 - gameOverSurface->w / 2-10, SCREEN_HEIGHT / 2-50, gameOverSurface->w+20, gameOverSurface->h+30};
    ...
    exitButton.isClicked = false;
    // Change positions of the buttons here
    exitButton.rect.x = SCREEN_WIDTH / 2+20; // Adjust 'No' button's X position
    exitButton.rect.y = SCREEN_HEIGHT / 2+40; // Adjust 'No' button's Y position
    drawButton(exitButton, "No");

    restartButton.isClicked = false;
     restartButton.rect.x = SCREEN_WIDTH / 2 -110; // Adjust 'Yes' button's X position
    restartButton.rect.y = SCREEN_HEIGHT / 2+40 ; // Adjust 'Yes' button's Y position
